...
//            ServerConfigurationManager, Packet27Position, WorldServer, Packet10Flying, 
//            Entity, AxisAlignedBB, ItemInWorldManager, Packet13PlayerLookMove, 
//            Packet14BlockDig, WorldProvider, ChunkCoordinates, MathHelper, 
//            Packet53BlockChange, InventoryPlayer, Packet15Place, ItemStack, 
//            Container, Packet103SetSlot, Slot, Packet16BlockItemSwitch, 
//            ChatAllowedCharacters, DamageSource, ModLoaderMp, Packet18Animation, 
//            Packet19EntityAction, Packet7UseEntity, Packet102WindowClick, Packet106Transaction, 
//            Packet108EnchantItem, Packet107CreativeSetSlot, Item, Packet130UpdateSign, 
//            TileEntitySign, TileEntity, Packet, Packet9Respawn, 
    ...
    implements ICommandListener
{
    ...
    {
        playerEntity.setMovementType(packet27position.getStrafeMovement(), packet27position.getForwardMovement(), packet27position.isSneaking(), packet27position.isInJump(), packet27position.getPitchRotation(), packet27position.getYawRotation());
    }
    ...
    private void handleSlashCommand(String s)
    {
        ...
        if(ModLoaderMp.HandleCommand(s.substring(1), playerEntity.username, logger, mcServer.configManager.isOp(playerEntity.username)))
        {
    ...
    public void handleEntityAction(Packet19EntityAction packet19entityaction)
    {
        ...
        if(packet19entityaction.state == 4)
        {
            playerEntity.setSprinting(true);
        } else
        if(packet19entityaction.state == 5)
        {
