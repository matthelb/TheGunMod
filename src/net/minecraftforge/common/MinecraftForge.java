...
public class MinecraftForge {
	public static boolean GUN_CREATOR_SRC_MOD;
	/**
	 * The core Forge EventBusses, all events for Forge will be fired on these,
	 * you should use this to register all your listeners. This replaces every
	 * register*Handler() function in the old version of Forge. TERRAIN_GEN_BUS
	 * for terrain gen events ORE_GEN_BUS for ore gen events EVENT_BUS for
	 * everything else
	 */
	 ...
	 * 
	 * @param tool
	 *            The custom tool to register.
	 * @param toolClass
	 *            The tool class to register as. The predefined tool clases are
	 *            "pickaxe", "shovel", "axe". You can add others for custom
	 *            tools.
	 * @param harvestLevel
	 *            The harvest level of the tool.
	 */
	public static void setToolClass(Item tool, String toolClass,
			int harvestLevel) {
		ForgeHooks.toolClasses
				.put(tool, Arrays.asList(toolClass, harvestLevel));
	}

	/**
	 * Register a block to be harvested by a tool class. This is the metadata
	 * sensitive version, use it if your blocks are using metadata variants. By
	 * default, this sets the block class as effective against that type.
	 * 
	 * @param block
	 *            The block to register.
	 * @param metadata
	 *            The metadata for the block subtype.
	 * @param toolClass
	 *            The tool class to register as able to remove this block. You
	 *            may register the same block multiple times with different tool
	 *            classes, if multiple tool types can be used to harvest this
	 *            block.
	 * @param harvestLevel
	 ...
	 */
	public static void setBlockHarvestLevel(Block block, int metadata,
	...
	/**
	 * Remove a block effectiveness mapping. Since setBlockHarvestLevel makes
	 * the tool class effective against the block by default, this can be used
	 * to remove that mapping. This will force a block to be harvested at the
	 * same speed regardless of tool quality, while still requiring a given
	 * harvesting level.
	 * 
	 * @param block
	 *            The block to remove effectiveness from.
	 * @param metadata
	 *            The metadata for the block subtype.
	 * @param toolClass
	 *            The tool class to remove the effectiveness mapping from.
	 * @see MinecraftForge#setToolClass for details on tool classes.
	 */
	public static void removeBlockEffectiveness(Block block, int metadata,
			String toolClass) {
		List key = Arrays.asList(block, metadata, toolClass);
	...
	/**
	 * Register a block to be harvested by a tool class. By default, this sets
	 * the block class as effective against that type.
	 * 
	 * @param block
	 *            The block to register.
	 * @param toolClass
	 ...
	 * @see MinecraftForge#setToolClass for details on tool classes.
	 */
	public static void setBlockHarvestLevel(Block block, String toolClass,
			int harvestLevel) {
	...
	public static void initialize() {
		...
		System.arraycopy(EntityEnderman.carriableBlocks, 0, temp, 0,
				EntityEnderman.carriableBlocks.length);
